...
- [x] **Metrics-first persistent summary**: a summary strip (cost range, timeline, compliance flags) visible across all tabs (2D/3D/Schedule/Export), not buried in the "Structural Engineering & BOQ Costing" tab alone — reinforces the product's core identity (quantified, actionable output) everywhere in the UI.
